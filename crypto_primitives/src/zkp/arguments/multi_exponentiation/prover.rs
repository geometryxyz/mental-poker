...
use crate::utils::rand::{RandomSampler, ScalarSampler};
use crate::utils::vector_arithmetic::{DotProduct, DotProductCalculator};
...
use crate::homomorphic_encryption::{el_gamal::ElGamal, HomomorphicEncryptionScheme};
use ark_ff::{One, Zero};
use merlin::Transcript;
use rand::Rng;

pub struct Prover<'a, C, Enc, Comm>
where
    C: ProjectiveCurve,
    Enc: HomomorphicEncryptionScheme,
    Comm: HomomorphicCommitmentScheme,
{
    parameters: &'a Parameters<'a, C, Enc, Comm>,
    transcript: Transcript,
    statement: &'a Statement<'a, C, Enc, Comm>,
    witness: &'a Witness<'a, C, Enc, Comm>,
    _encryption_scheme: PhantomData<Enc>,
    _commitment_scheme: PhantomData<Comm>,
}

impl<'a, C, Enc, Comm> Prover<'a, C, Enc, Comm>
where
    C: ProjectiveCurve,
    Enc: HomomorphicEncryptionScheme,
    Comm: HomomorphicCommitmentScheme,
{
    ...
    pub fn prove<R: Rng>(
        &self,
        rng: &mut R,
        encryption_parameters: &Enc::Parameters,
    ) -> Proof<C, Enc> {
        ...
        let commit_b_k = b
            .iter()
            .zip(s.iter())
            .map(|(b_k, s_k)| Comm::commit(self.parameters.commit_key, vec![b_k], *s_k))
            .collect::<Vec<_>>();
