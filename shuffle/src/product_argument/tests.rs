...
mod test {
    ...
    use crate::product_argument::{Witness, prover::Prover, Parameters, Statement};
    use rand::Rng;
    ...
    fn test_complete_product_argument() {
        ...
        let new_random_scalars = ScalarSampler::<starknet_curve::Projective>::sample_vector(rng, m*n);
        let bad_a_chunks = new_random_scalars.chunks(n).map(|c| c.to_vec()).collect::<Vec<_>>();

        let bad_r = ScalarSampler::<starknet_curve::Projective>::sample_vector(rng, a_chunks.len());
        let bad_a_commits = a_chunks.iter().zip(bad_r.iter()).map(|(a_chunk, &random)|{
            PedersenCommitment::commit_vector(&commit_key, a_chunk, random)
        }).collect::<Vec<starknet_curve::Projective>>();
        ...
        assert_ne!(Ok(()), invalid_proof.verify(&proof_parameters, &wrong_statement));
    }
}