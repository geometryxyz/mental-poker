...
use crate::{
    ...
    utils::{
        DotProduct, DotProductCalculator, HadamardProduct, HadamardProductCalculator,
        HomomorphicCommitment, PedersenCommitment, RandomSampler, ScalarSampler,
    },
};
...
impl<'a, C: ProjectiveCurve> Prover<'a, C> {
    ...
    pub fn prove<R: Rng>(&self, rng: &mut R) -> Proof<C> {
        ...
        let vec_commits_to_a: Vec<C> = [&self.statement.commitment_to_a[..], &[minus_ones_commit]]
            .concat()
            .to_vec();
        ...
        let vec_openings_to_a = [&self.witness.matrix_a[1..], &[vec_minus_ones]]
            .concat()
            .to_vec();
        let vec_randoms_for_a = [
            &self.witness.randoms_for_a_commit[1..],
            &[C::ScalarField::zero()],
        ]
        .concat()
        .to_vec();

        let final_t = DotProductCalculator::<C>::scalars_by_scalars(
        ...
        let temp_x_b = b[1..=self.parameters.m - 1]
            ...
            .map(|(b_chunk, &x_power_i)| {
                ...
                HadamardProductCalculator::<C>::scalars_by_scalars(b_chunk, &x_power_i_vector)
                    .unwrap()
            })
        ...
        let zero_arg_statement =
            zero_argument::Statement::<C>::new(&vec_commits_to_a_shifted, &c_d_i, &prover_mapping);
        let zero_arg_params = zero_argument::Parameters::<C>::new(
            self.parameters.m,
            self.parameters.n,
            &self.parameters.commit_key,
        );
        let zero_arg_witness = zero_argument::Witness::<C>::new(
            &vec_openings_to_a,
            &vec_randoms_for_a,
            &vec_openings_to_d,
            &vec_randoms_for_d,
        );
        let zero_arg_prover = zero_argument::prover::Prover::<C>::new(
            &zero_arg_params,
            &zero_arg_statement,
            &zero_arg_witness,
        );
