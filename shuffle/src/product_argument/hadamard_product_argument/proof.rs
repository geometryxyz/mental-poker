...
use crate::{
    utils::{
        PedersenCommitment, HomomorphicCommitment,
    }, 
    error::Error,
...
impl<C:ProjectiveCurve> Proof<C> {
    pub fn verify(&self, proof_parameters: &Parameters<C>, statement: &Statement<C>) -> Result<(), Error> {
        ...
        .chain(iter::once(x))
        .chain(
            (1..proof_parameters.m-1).scan(x, |current_power, _exp| {
                *current_power *= x;
                Some(*current_power)
            })
        )
        ...
        let mut c_d_i = self.b_commits.iter().zip(x_challenge_powers.iter().skip(1)).map(|(&b_i_commit, &x_power_i)| {
            b_i_commit.mul(x_power_i.into_repr())
        }).collect::<Vec<C>>();

        let temp_x_c_d_shifted = self.b_commits.iter().skip(1).zip(x_challenge_powers.iter().skip(1)).map(|(&b_i_commit, &x_power_i)| {
            b_i_commit.mul(x_power_i.into_repr())
        }).collect::<Vec<C>>();
        ...
        let zero_arg_params = zero_argument::Parameters::<C>::new(proof_parameters.m, proof_parameters.n, &proof_parameters.commit_key);
        
        let minus_one = -C::ScalarField::one();
        let vec_minus_ones = vec![minus_one; proof_parameters.n];
        let minus_ones_commit = PedersenCommitment::<C>::commit_vector(proof_parameters.commit_key, &vec_minus_ones, C::ScalarField::zero());
        let vec_commits_to_a: Vec<C> = [&statement.commitment_to_a[1..], &[minus_ones_commit]].concat().to_vec();

        let zero_arg_statement = zero_argument::Statement::<C>::new(&vec_commits_to_a, &c_d_i, &prover_mapping);

        match self.zero_arg_proof.verify(&zero_arg_params, &zero_arg_statement) {
            Ok(_) => Ok(()),
