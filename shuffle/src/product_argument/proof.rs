...
use crate::product_argument::{Statement, Parameters};
        ...
        C: ProjectiveCurve
{
    pub fn verify(&self, proof_parameters: &Parameters<C>, statement: &Statement<C>) -> Result<(), Error> {
        ...
        let hadamard_product_statement = hadamard_product_argument::Statement::new(
            statement.commitments_to_a,
            self.b_commit
        );

        self.hadamard_product_proof.verify(&hadamard_product_parameters, &hadamard_product_statement)?;
        ...
        self.single_value_proof.verify(&single_value_product_parameters, &single_value_product_statement)?;
