...
impl<C: ProjectiveCurve> Proof<C> {
    pub fn verify(
        ...
        let challenge_powers = iter::once(C::ScalarField::one())
            .chain(iter::once(x))
            .chain((1..2 * proof_parameters.m).scan(x, |current_power, _exp| {
                *current_power *= x;
                Some(*current_power)
            }))
            .collect::<Vec<_>>();
        ...
        let left: C = self.a_0_commit
            + DotProductCalculator::<C>::scalars_by_points(
        ...
        let left = DotProductCalculator::<C>::scalars_by_points(
            &challenge_powers,
            &self.vector_of_commited_diagonals,
        )
        .unwrap();
