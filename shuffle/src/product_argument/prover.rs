use super::{Statement, Parameters, Witness, proof::Proof};

use crate::{
    utils::{
        ScalarSampler, RandomSampler, PedersenCommitment, HomomorphicCommitment, HadamardProduct, HadamardProductCalculator
    }, 
    product_argument::*,
};
...
impl<'a, C: ProjectiveCurve> Prover<'a, C> {
    ...
    pub fn prove<R: Rng>(&self, rng: &mut R) -> Proof<C> {
        ...
        let product_along_rows = self.witness.matrix_a.iter().fold(vec![C::ScalarField::one(); self.parameters.n], |x, y| {
            HadamardProductCalculator::<C>::scalars_by_scalars(&x, &y).unwrap()
        });

        let b_commit = PedersenCommitment::<C>::commit_vector(self.parameters.commit_key, &product_along_rows, s);
        ...
        let hadamard_product_statement = hadamard_product_argument::Statement::new(
            &self.statement.commitments_to_a,
            b_commit
        );
        ...
        let single_value_product_witness = single_value_product_argument::Witness::<C>::new(
            &product_along_rows,
            &s
        );

        let single_value_product_statement = single_value_product_argument::Statement::<C>::new(
            b_commit, 
            self.statement.b
        );
