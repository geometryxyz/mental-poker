...
mod test {
    use crate::{prover::Prover, Parameters, Permutation, Statement, Witness};
    ...
    use verifiable_threshold_masking_protocol::discrete_log_vtmp::{
        DiscreteLogVTMF, VerifiableThresholdMaskingProtocol,
    };
    ...
    fn test_shuffle_argument() {
        ...
        let (pk, _) =
            DiscreteLogVTMF::<starknet_curve::Projective>::keygen(&elgamal_parameters, rng)
                .unwrap();
        ...
        let witness =
            Witness::<starknet_curve::Projective>::new(&permutation, &masking_factors_raw);

        let honest_prover =
            Prover::<starknet_curve::Projective, ElGamal<starknet_curve::Projective>>::new(
                &parameters,
                &statement,
                &witness,
            );
        ...
        assert_eq!(
            Ok(()),
            valid_proof.verify(&parameters, &statement, &elgamal_parameters)
        );

        let new_permutation = Permutation::new(rng, number_of_cards);
        let bad_witness =
            Witness::<starknet_curve::Projective>::new(&new_permutation, &masking_factors_raw);

        let malicious_prover = Prover::<
            starknet_curve::Projective,
            ElGamal<starknet_curve::Projective>,
        >::new(&parameters, &statement, &bad_witness);

        let invalid_proof = malicious_prover.prove(rng, &elgamal_parameters);
        assert_ne!(
            Ok(()),
            invalid_proof.verify(&parameters, &statement, &elgamal_parameters)
        );
    }
}
