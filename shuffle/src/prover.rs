    ...
    Prover<'a, C, EncryptionScheme>
{
    ...
    pub fn prove<R: Rng>(
        ...
        let a_commits = a_chunks
            .iter()
            .zip(r.iter())
            .map(|(a, &r)| PedersenCommitment::<C>::commit_vector(self.parameters.commit_key, a, r))
            .collect::<Vec<C>>();
        ...
        let challenge_powers = iter::once(x)
            .chain(
        ...
        let b_commits = b_chunks
            .iter()
            .zip(s.iter())
            .map(|(b, &s)| PedersenCommitment::<C>::commit_vector(self.parameters.commit_key, b, s))
            .collect::<Vec<C>>();
        ...
        let temp =
            DotProductCalculator::<C>::scalars_by_ciphers(&b, self.statement.shuffled_ciphers)
                .unwrap();
        ...
        let multi_exp_statement =
            multi_exponent_argument::Statement::<C>::new(&shuffled_chunks, product, &b_commits);

        let multi_exp_witness = multi_exponent_argument::Witness::<C>::new(&b_chunks, &s, rho);
