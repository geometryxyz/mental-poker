...
impl<'a, C: ProjectiveCurve, EncryptionScheme: AsymmetricEncryptionScheme> Prover<'a, C, EncryptionScheme> {
    ...
    pub fn prove<R: Rng>(&self, rng: &mut R, encryption_parameters: &ElGamalParameters<C>) -> Proof<C> {
        ...
        transcript.append(b"commitments_to_exponents", self.statement.commitments_to_exponents);
        ...
        let commit_b_k = b.iter().zip(s.iter()).map(|(b_k, s_k)| {
            PedersenCommitment::<C>::commit_scalar(self.parameters.commit_key[0], *self.parameters.commit_key.last().unwrap(), *b_k, *s_k)
        }).collect::<Vec<_>>();

        let diagonals = Self::diagonals_from_chunks(&self.statement.shuffled_ciphers, &self.witness.matrix_a, &a_0).unwrap();

        let vector_e_k = b.iter().zip(tau.iter()).zip(diagonals.iter()).map(|((b_k, tau_k), d_k)| {
            let encrypted_random = DiscreteLogVTMF::<C>::mask(encryption_parameters, self.parameters.public_key, &self.parameters.masking_generator.mul(*b_k).into_affine(), &Randomness::<C>(*tau_k));
        ...
        let scalar_products_ax = self.witness.matrix_a.iter().enumerate().map(|(i, chunk)|{
            chunk.iter().map(|scalar|{x_array[i] * scalar}).collect::<Vec<_>>()
        ...
        let r_blinded = r_0 + DotProductCalculator::<C>::scalars_by_scalars(&self.witness.matrix_blinders, &x_array).unwrap();
        let b_blinded = DotProductCalculator::<C>::scalars_by_scalars(&b, &challenge_powers).unwrap();
        let s_blinded = DotProductCalculator::<C>::scalars_by_scalars(&s, &challenge_powers).unwrap();
        let tau_blinded = DotProductCalculator::<C>::scalars_by_scalars(&tau, &challenge_powers).unwrap();
    ...
    fn diagonals_from_chunks(
     ...
     -> Result<Vec<ElgamalCipher<C>>, Error> {
        ...
        let product: ElgamalCipher<C> = cipher_chunks.iter().zip(scalar_chunks.iter()).map(|(c_i, a_i)| {
            DotProductCalculator::<C>::scalars_by_ciphers(a_i, c_i).unwrap()
        }).sum();
...
mod test {
    ...
    use ark_std::{rand::{thread_rng, seq::SliceRandom}, UniformRand};
    ...
    use crate::{
        utils::{RandomSampler, ScalarSampler, PointSampler, HomomorphicCommitment, PedersenCommitment}
    };
    ...
    fn proof_verification_test() {
        ...
        let (master_pk, _) = DiscreteLogVTMF::<Projective>::keygen(&elgamal_parameters, rng).unwrap();
        ...
        let deck_of_cards = card_attributes.iter().zip(masking_factors.iter()).map(|(attribute, masking_factor)| {
            DiscreteLogVTMF::<Projective>::mask(&elgamal_parameters, &master_pk, &attribute.into_affine(), &Randomness(*masking_factor)).unwrap()
        }).collect::<Vec<_>>();
        ...
        let a_chunks = shuffle_maskings.chunks(n).map(|c| c.to_vec()).collect::<Vec<_>>();
        let shuffle_maskings_to_rand = shuffle_maskings.iter().map(|x| Randomness(*x)).collect::<Vec<_>>();

        let shuffled_deck = DiscreteLogVTMF::<Projective>::mask_shuffle(&elgamal_parameters, &master_pk, &deck_of_cards, &shuffle_maskings_to_rand, &permutation).unwrap();
        let c_chunks = shuffled_deck.chunks(n).map(|c| c.to_vec()).collect::<Vec<_>>();
        ...
        let c_a = a_chunks.iter().zip(r.iter()).map(|(a_chunk, random)|{
        ...
        assert_eq!(Ok(()), proof.verify(&proof_parameters, &elgamal_parameters, &statement));
    }
}