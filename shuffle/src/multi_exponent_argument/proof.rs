...
impl<C: ProjectiveCurve> Proof<C> {
    pub fn verify(&self, proof_parameters: &Parameters<C>, encryption_parameters: &ElGamalParameters<C>, statement: &Statement<C>) -> Result<(), Error>{
        ...
        transcript.append(b"commitments_to_exponents", statement.commitments_to_exponents);
        ...
        let aggregate_masking_cipher = DiscreteLogVTMF::<C>::mask(
            ...
            &proof_parameters.masking_generator.mul(self.b_blinded.into_repr()).into_affine(),
        ...
        */
        ...
        let verif_rhs: ElgamalCipher<C> = challenge_powers.iter().take(m).rev().zip(statement.shuffled_ciphers.iter()).map(|(power_of_x, cipher_chunk)|{
            // x^m - i * a_vec
            let xm_minus_i_times_a = self.a_blinded.iter().map(|element_of_a|{
