...
mod test {
	...
	fn generate_and_verify_key() {
		...
		assert_eq!(
			...
			Err(CryptoError::ProofVerificationError(String::from("Schnorr Identification")))
		)
	}
	...
	fn aggregate_keys() {
		...
		assert_eq!(
			test_fail_aggregate,
			Err(CardProtocolError::ProofVerificationError(CryptoError::ProofVerificationError(
				String::from("Schnorr Identification")
			)))
		)
	}
	...
	fn test_shuffle() {
		...
		assert_eq!(
			...
			Err(CryptoError::ProofVerificationError(String::from("Hadamard Product (5.1)")))
		)
	}
}
